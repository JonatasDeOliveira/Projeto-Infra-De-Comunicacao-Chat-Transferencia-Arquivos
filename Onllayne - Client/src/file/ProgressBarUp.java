...
public class ProgressBarUp implements Runnable {
	...
	public void run(){
		...
		while(true){
			...
			label1.setText("Tempo final: " + (int)this.fileSender.getRemainingTime() + " s");
			lblMbs.setText("Velocidade: " + formater.format(this.fileSender.getSpeed()) + " MB/s");
			if((timeAfter - timeInit) >= 1000){
				lblEstRTT.setText("RTT Estimado: " + formater.format(this.fileSender.getEstimatedRTT()/1000000) + " ms");

...
public class Control extends Thread {
	...
	private void controlPush(byte[] b) {
		...
		if(chat != null){
			...
			for(int i=0 ; i<numUser ; i++){
				hsUser.add(in.nextLine());
			}
			chat.setUsers(hsUser);
			Cryptography cy = new Cryptography(chat.getId());
			while(true){
				String str = in.nextLine();
				if(str.equals("$term")) break;
	...
	private void controllistUserRep(byte[] b) {
		...
		while(true){
			...
			if(!str.substring(0, str.indexOf(' ')).equals(this.user.getNickname())){
				JLabel lblNewLabel = new JLabel();
				lblNewLabel.setForeground(Color.WHITE);
				lblNewLabel.setFont(new Font("Dialog", Font.PLAIN, 14));
				lblNewLabel.setBorder(new EmptyBorder(0, 4, 0, 0));
				lblNewLabel.setOpaque(true);
				lblNewLabel.setText("<html><b>" + str.substring(0, index) + "</b>" + str.substring(index, str.length()) + "</html>");
	...
	public void controlLogInDone(byte [] b){
		...
		String path = "rep//" + this.user.getNickname() + "//";

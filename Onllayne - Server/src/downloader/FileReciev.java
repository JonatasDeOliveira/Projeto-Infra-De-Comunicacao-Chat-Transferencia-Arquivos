...
public class FileReciev implements Runnable {
	...
	public FileReciev(String path, long fileSize, Control control, String nickname, String id, int port) throws UnknownHostException, IOException {
		...
		this.port = port;
		this.portACK = port + 1;
		this.file = new FileReceiver(this.path, this.port);
		this.fileSize = fileSize;
		this.ack = new ACKReciev(0);
	}
	...
	public void run() {
		try {
			...
			BufferedOutputStream bosACK = new BufferedOutputStream(soc.getOutputStream());
			...
			while (flag) {
				try {
					...
					// if((System.currentTimeMillis()-ini) >= 21000 && cond)
					// isCanceled = true;
					...
					if (isCanceled()) {
						...
						while (isPaused()) {
							...
							// while((System.currentTimeMillis()-ini) < 7000 &&
							// cond) {}
							// this.isPaused = false;
							// cond = false;
						}
						System.out.println("to resumindo");
					...
					while (true) {
						try {
							...
							bytesTime -= sizeA;
							sizeA = 0;
							bosACK = new BufferedOutputStream(soc.getOutputStream());
			...
			//System.out.println("olaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa");
			ack.destroySock();
			this.ack.stop();
			this.file.EOF();
			bis.close();
			...
			File myFile = this.file.getFile();
			this.control.finalizeFileUpload(myFile.getName(), myFile, this.fileSize, nickname, id, this.port);
		} catch (Exception e) {
		}
	}
